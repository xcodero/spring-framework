...
public class LoadTimeWeaverAwareProcessor implements BeanPostProcessor, BeanFactoryAware {
	...
	public Object postProcessBeforeInitialization(Object bean, String beanName) throws BeansException {
		// 只处理实现了LoadTimeWeaverAware接口的类型的bean（如AspectJWeavingEnabler类型的bean）
		if (bean instanceof LoadTimeWeaverAware) {
			LoadTimeWeaver ltw = this.loadTimeWeaver;
			if (ltw == null) {
				Assert.state(this.beanFactory != null,
						"BeanFactory required if no LoadTimeWeaver explicitly specified");
				// 从bean工厂中获取LoadTimeWeaver类型的bean
				ltw = this.beanFactory.getBean(
						ConfigurableApplicationContext.LOAD_TIME_WEAVER_BEAN_NAME, LoadTimeWeaver.class);
			}
			// 将获取的bean设置到AspectJWeavingEnabler类型bean的loadTimeWeaver属性中
